#!/usr/bin/env python3
"""Scrape student information from the MADSys lab website."""
...
BASE_URL = "https://madsys.cs.tsinghua.edu.cn/"
PEOPLE_URL = urljoin(BASE_URL, "people/")
TARGET_GROUPS = {"Ph.D. Students", "Master Students"}
PROFILE_SECTIONS = {
    "bio": "bio",
    "education": "education",
...
USER_AGENT = (
    "Mozilla/5.0 (compatible; MADSysStudentScraper/1.0; "
...
def parse_profile(session: requests.Session, profile_url: str) -> dict[str, str]:
    ...
    result = {
        ...
        "official_external_links": "",
    }

    portrait_title = soup.select_one(".portrait-title")
    if portrait_title is not None:
        h3_texts = [text_from_node(tag) for tag in portrait_title.select("h3") if text_from_node(tag)]
        if len(h3_texts) >= 2:
            result["lab"] = h3_texts[-1]

    external_links: list[str] = []
    for link in soup.select(".network-icon a[href]"):
        ...
        elif absolute_url != profile_url and not result["personal_website_url"]:
            result["personal_website_url"] = absolute_url

    if external_links:
        result["official_external_links"] = " | ".join(dict.fromkeys(external_links))

    for heading in soup.select(".section-subheading"):
        ...
        if container is not None:
            for child in container.children:
                if not isinstance(child, Tag) or child is heading:
                    continue
                blocks.append(child)
        result[PROFILE_SECTIONS[title]] = flatten_blocks(blocks)
...
def parse_people_page(session: requests.Session) -> list[StudentRecord]:
    ...
    people_section = soup.select_one("section#people")
    if people_section is None:
        raise RuntimeError("Could not locate the People section on the MADSys site.")

    students: list[StudentRecord] = []
    current_group = ""
    for heading in people_section.select("h2"):
        title = text_from_node(heading)
        if not title:
            continue

        if title in {"People", "Faculty", "Alumni"}:
            current_group = title
            continue
        ...
        if current_group not in TARGET_GROUPS:
            continue
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output",
        default="madsys_students.csv",
        help="Path to the CSV output file. Default: %(default)s",
    )
    parser.add_argument(
        "--xlsx",
        default="madsys_students.xlsx",
...
def main() -> int:
    ...
    if not records:
        raise RuntimeError("No students were scraped from the MADSys people page.")
